...
test.skip("deposits to the solend main pool", async () => {
   ...
   */
  const maximumDeposit = await getMaximumDeposit({
    connection,
    environment: "devnet",
    symbol: "SOL"
  })
  ...
  const depositAction = await deposit({
    ...
    amount: depositAmount.subunits, // TODO: For other tokens, should this be in lamports?
    symbol: "SOL"
  })
